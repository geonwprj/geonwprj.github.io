    ...
    // findRoutes(fromLat, fromLong, toLat, toLong).then(routes => {
        ...
        if (routes.length === 0 || !fromLat) {
            ...
            const formattedResults = routes.map(v => ({
                ...
                fromStop: v.fromStop.stop,
                fromStopName: v.fromStop.name_tc,
                fromStopDistance: v.fromStop.distance,
                toStop: v.toStop.stop,
                toStopName: v.toStop.name_tc,
                toStopDistance: v.toStop.distance,
            }));
